...
export function WorkbookBuilder() {
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      // 성공 시엔 리셋하지 않는다 — push 후 페이지를 떠날 때까지 버튼을 잠가
      // 재클릭으로 문제집이 중복 생성되는 것을 막는다(예전 createdWorkbookId 가드의 역할 대체).
    } catch (e) {
      console.error("문제집 생성 실패:", e);
      toast.error(e instanceof Error ? e.message : "문제집 생성에 실패했습니다.");
      setCreatingWorkbook(false); // 실패했을 때만 다시 시도할 수 있게 잠금 해제.
    }
  };
  ...
  return (
    <div className="mx-auto max-w-[980px] px-4 py-8 md:px-8">
      ...
      {/* ── 과목 선택 (다중) — 선택 즉시 문제집을 만들고 바로 /edit로 이동하므로
       *    단계가 하나뿐이다(예전 "2 생성 방식" 스테퍼는 렌더될 수 없는 죽은 UI라 제거). ── */}
