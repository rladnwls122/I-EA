...
describe('GeminiLlmService — 자기검증', () => {
  ...
  describe('판정 프롬프트', () => {
    it('코드가 못 잡는 4축만 판정하게 하고, 형식 규격은 다시 보지 말라고 못 박는다', async () => {
      ...
      await service.reviewGeneration(ctx, result);
  ...
  describe('판정 파서', () => {
    it('PASS 판정을 그대로 돌려준다', async () => {
      ...
      const { verdicts } = await service.reviewGeneration(ctx, result);
    ...
    it('문항 수와 판정 수가 다르면 예외 — "판정했다"고 기록할 수 없다', async () => {
      ...
      await expect(service.reviewGeneration(ctx, result)).rejects.toThrow(/판정하지 않았습니다/);
    });

    it('알 수 없는 판정값은 예외', async () => {
      const service = await makeService();
      spyCall(service, JSON.stringify({ verdicts: [{ index: 0, verdict: 'MAYBE' }] }));
      await expect(service.reviewGeneration(ctx, result)).rejects.toThrow(/판정값이 잘못/);
    });

    it('JSON이 아니면 예외', async () => {
      const service = await makeService();
      spyCall(service, '판정을 할 수 없습니다');
      await expect(service.reviewGeneration(ctx, result)).rejects.toThrow(/JSON을 찾지 못했습니다/);
    });
  });
});
