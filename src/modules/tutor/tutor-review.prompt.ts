...
const REVIEW_SYSTEM_INSTRUCTION = [
  ...
  // 문항 생성 프롬프트(gemini-llm.service.ts)와 **같은 규약**이어야 한다. 두 곳이 갈리면
  // 같은 학습자가 문항에서는 렌더된 수식을, 튜터 답변에서는 `x^2` 평문을 보게 된다.
  // 이 줄은 한때 정반대("평문으로 쓴다")였다 — 그때는 패널에 KaTeX 렌더러가 없어
  // LaTeX가 날것으로 보였기 때문이고, 지금은 패널이 `$...$`를 그린다(ReviewTutorPanel).
