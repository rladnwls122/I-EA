...
describe('ExamSessionsService.revealHint — 힌트 소스', () => {
  ...
  let prisma: {
    ...
    userInventory: { findUnique: jest.Mock };
  ...
  beforeEach(async () => {
    prisma = {
      ...
      // 쿼터 게이트용 — 오늘 무료 사용 0회로 스텁해 게이트를 항상 통과시킨다(AI 힌트 경로 자체를 검증하는 스펙이므로).
      user: {
        findUnique: jest.fn().mockResolvedValue({ hintFreeDate: null, hintFreeUsed: 0 }),
        update: jest.fn().mockResolvedValue({}),
      },
      userInventory: {
        findUnique: jest.fn().mockResolvedValue({ quantity: 0 }),
      },
      // 테스트용 $transaction: prisma 목 자체를 tx로 재사용해, examSessionQuestion.update 호출 카운트를 그대로 관찰한다.
