지금까지 대화를 통해 도출된 **보안 정책(Bcrypt), 별점/체감 난이도 이원화, 핀포인트 드래그 오답노트, 힌트 추적 기능, 검색 로그 및 조회수 캐시** 등의 모든 개선 사항을 반영하여 원본 `exam_studio_db_schema.md` 파일을 최신 사양으로 완전히 고도화했습니다.

이 내용을 그대로 복사하여 기존 마크다운 문서를 업데이트하시면 됩니다.
