...
백엔드는 **Vercel 서버리스**다(`railway.json`과 `start:railway`는 Railway를 떠나면서 삭제됐다 — 옛 문서가 말하는 그 경로는 더 이상 없다). **운영 스키마 동기화는 마이그레이션이 아니라 `db push`다**: `vercel.json`의 `buildCommand`가 `VERCEL_ENV=production`일 때만 `prisma db push --skip-generate`를 돌린다. 프리뷰에서는 건너뛴다 — 프리뷰도 같은 `DATABASE_URL`을 보므로, 건너뛰지 않으면 기능 브랜치의 스키마가 운영 DB로 밀려 들어간다. TiDB Serverless는 유휴 상태에서 첫 연결을 거절하므로(P1001) 한 번만 재시도한다. `--accept-data-loss`는 일부러 빼 두었다: 스키마 드리프트가 있으면 컬럼을 조용히 지우는 대신 배포가 요란하게 실패한다. 실패하는 배포를 잠재우려고 도로 넣지 말고 드리프트를 고쳐라. `prisma/migrations`의 dev 흐름과 배포 경로는 다르므로 `schema.prisma`를 정본으로 유지한다. Vercel에는 프로젝트가 두 개다: 백엔드 `i-ea` (`https://i-ea.vercel.app`, `vercel.json`이 모든 경로를 `api/index.js` 서버리스 함수로 rewrite)와 프런트엔드 `i-ea-web` (`https://i-ea-web.vercel.app`, Root Directory = `web`). 프런트의 `NEXT_PUBLIC_API_URL`은 `/api`까지 포함한 백엔드 주소여야 한다 (`lib/api.ts` 폴백이 `http://localhost:3000/api`이고, `next.config.mjs`가 이 값에서 CSP `connect-src` 오리진을 뽑는다). 빌드 타임 값이라 바꾸면 프런트를 재배포해야 반영된다.  한때 Cloudflare Pages로 배포하려던 시도가 있었으나 폐기됐다. 그 잔재였던 `@cloudflare/next-on-pages`/`wrangler`(및 설정 없이 남아 있던 `orval`)는 제거했다 — 되살리지 말 것.
