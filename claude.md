...
Railway via `railway.json` → `npm run start:railway`, which runs `prisma db push --skip-generate` then `node dist/main.js`. **Production uses `db push`, not migrations** — the `prisma/migrations` dev flow and the deployed schema-sync path differ; keep `schema.prisma` authoritative. `--accept-data-loss` was deliberately dropped: on schema drift the deploy now fails loudly instead of silently dropping columns. Don't add it back to quiet a failing deploy — fix the drift. Vercel에는 프로젝트가 두 개다: 백엔드 `i-ea` (`https://i-ea.vercel.app`, `vercel.json`이 모든 경로를 `api/index.js` 서버리스 함수로 rewrite)와 프런트엔드 `i-ea-web` (`https://i-ea-web.vercel.app`, Root Directory = `web`). 프런트의 `NEXT_PUBLIC_API_URL`은 `/api`까지 포함한 백엔드 주소여야 한다 (`lib/api.ts` 폴백이 `http://localhost:3000/api`이고, `next.config.mjs`가 이 값에서 CSP `connect-src` 오리진을 뽑는다). 빌드 타임 값이라 바꾸면 프런트를 재배포해야 반영된다.  한때 Cloudflare Pages로 배포하려던 시도가 있었으나 폐기됐다. 그 잔재였던 `@cloudflare/next-on-pages`/`wrangler`(및 설정 없이 남아 있던 `orval`)는 제거했다 — 되살리지 말 것.
