...
- 인증은 이메일 + 비밀번호(bcrypt 12라운드)다. `auth.service.ts`의 `register`/`login`, `passwordHash` 컬럼. 토큰 무효화는 `users.token_version`(JWT의 `tv` 클레임)으로 하고 `POST /auth/logout-all`이 올린다. 인증 라우트에는 레이트리밋이 걸려 있다(로그인 5분 10회).
...
`README.md`·`LOCAL_TEST_GUIDE.md`·`ARCHITECTURE.md`는 현재 코드에 맞게 갱신돼 있다(units·enum `QuestionType`·variants/memos·비밀번호 없는 로그인 서술은 전부 제거됨). 다만 충돌이 보이면 여전히 **`schema.prisma`와 코드가 최종 진실**이고, 문서 쪽을 고쳐라.
